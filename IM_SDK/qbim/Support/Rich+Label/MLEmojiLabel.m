...
//REGULAREXPRESSION(AtRegularExpression, @"@[\\u4e00-\\u9fa5\\w\3-]+")
REGULAREXPRESSION(AtRegularExpression, @"")


//@"#([^\\#|.]+)#"
REGULAREXPRESSION_OPTION(PoundSignRegularExpression, @"#([\\u4e00-\\u9fa5\\.\\W\\s\\w\\-$]+)#", NSRegularExpressionCaseInsensitive)
...
- (NSDictionary*)emojiDictForKey:(NSString*)key
{
    ...
    NSDictionary* tmp = [[NSDictionary alloc] initWithContentsOfFile:emojiFilePath];
    NSDictionary* dic = [NSDictionary dictionaryWithObjects:tmp.allKeys forKeys:tmp.allValues];
...
@interface MLEmojiLabel()<NIMAttributedLabelDelegate>
...
//    [self setValue:[NSArray array] forKey:@"links"];
    ...
    UIColor *commonLinkColor = [SSIMSpUtil getColor:@"4077af"];
    
    //点击时候的背景色
    [mutableActiveLinkAttributes setValue:(__bridge id)[[UIColor colorWithWhite:0.631 alpha:1.000] CGColor] forKey:(NSString *)kNIMBackgroundFillColorAttributeName];
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns(line)) {
            ...
            if (imageName) {
                ...
                UIImage *image = IMGGET(imageName);
                runBounds.origin.y -= emojiOriginYOffset - 1; //稍微矫正下。
                NSLog(@"稍微矫正下");
...
- (void)setText:(id)text
{
    ...
    for (NSUInteger i=0; i<maxIndex; i++) {
        ...
        [regexps[i] enumerateMatchesInString:mutableAttributedString.string options:0 range:stringRange usingBlock:^(NSTextCheckingResult *result, __unused NSMatchingFlags flags, __unused BOOL *stop) {
            ...
            if([actionString hasPrefix:@"phoneNumber->"]){//去掉电话号码
                return;
            }
            ...
            if([actionString hasPrefix:@"poundSign->"]){
//                NSString* tmp = [actionString stringByReplacingOccurrencesOfString:@"poundSign->" withString:@""];
//                TopicEntity* top = [TopicEntity NIM_findFirstWithPredicate:[NSPredicate predicateWithFormat:@"name=%@",tmp]];
//                TopicEntity* top = nil;
//                if(top)
...
- (void)attributedLabel:(NIMAttributedLabel *)label didSelectLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
{
