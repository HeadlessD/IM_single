//
//  NIMJXNIMPublicSearchResultCell.m
//  QianbaoIM
//
...
#import "NIMJXPublicSearchResultCell.h"

@implementation NIMJXNIMPublicSearchResultCell
...
- (void)setCellDataSource:(publicMode*)publicmode searchString:(NSString*)searchString
{
    ...
    if(IsStrEmpty(searchString))
    {
        self.nameLable.text=publicmode.nickName;
    ...
    else
    {
        if (IsStrEmpty(publicmode.nickName)) {
            ...
            NSMutableAttributedString *attrituteString = [[NSMutableAttributedString alloc] initWithString:publicmode.nickName];
            NSRange range = [publicmode.nickName rangeOfString:searchString];
            [attrituteString setAttributes:@{NSForegroundColorAttributeName : [UIColor redColor],   NSFontAttributeName : [UIFont boldSystemFontOfSize:16]} range:range];
                NSRange range1 = NSMakeRange(0, attrituteString.length);
            NSDictionary *dic = [attrituteString attributesAtIndex:0 effectiveRange:&range1];
            
            CGSize theStringSize = [publicmode.nickName boundingRectWithSize:self.nameLable.bounds.size // 用于计算文本绘制时占据的矩形块
