    ...
    [self.keyItemGroups enumerateObjectsUsingBlock:^(WUEmoticonsKeyboardKeyItemGroup *obj, NSUInteger idx, BOOL *stop) {
        if (obj.image) {
            if (obj.selectedImage && (NSInteger)idx == self.segmentedControl.selectedSegmentIndex) {
                ...
                UIImage *onceActive = obj.selectedImage;
                if ([UIImage instancesRespondToSelector:@selector(imageWithRenderingMode:)]) {
                    onceActive = [obj.selectedImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
                }
                [self.segmentedControl setImage:onceActive forSegmentAtIndex:idx];
            } else {
                UIImage *onceActive = obj.image;
                if ([UIImage instancesRespondToSelector:@selector(imageWithRenderingMode:)]) {
                    onceActive = [obj.image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
                }
                [self.segmentedControl setImage:onceActive forSegmentAtIndex:idx];
            }
        } else {
