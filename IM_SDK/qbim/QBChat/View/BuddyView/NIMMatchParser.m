...
@interface NSNumber(rangeValue)

@end

@implementation NSNumber(rangeValue)

- (NSRange)rangeValue
{
    return NSMakeRange([self intValue], 0);
}

@end

@implementation NIMMatchParser
...
-(id)init
{
    ...
    if(self)
    {
        ...
        self.urlLink        =   YES;
        self.phoneLink      =   NO;
        self.nickNameLink   =   YES;
    }
    return self;
}

- (void)setData:(id<NIMMatchParserDelegate>)mdata
{
    data = mdata;
}
...
{
    if([SSIMSpUtil isEmptyOrNull:source])
        return;
    _source=source;
    source = [SSIMSpUtil trimString:source];
    ...
    for(int i=0;i<count;i++)
    {
        ...
        if(icon!=nil)
        {
            ...
            NSMutableDictionary * dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:iconStr,NIMMatchParserImage,[NSNumber numberWithInteger:[text length]],NIMMatchParserLocation,[NSNull null],NIMMatchParserRects, nil];
    ...
    {
        NSInteger location= [[dic objectForKey:NIMMatchParserLocation] integerValue];
    ...
    CFRelease(fontRef);
    CFRelease(style);
    if(self.sendNickName || self.toTragetName)
    {
        NSMutableAttributedString* _attrStr = attStr;
        
        if(self.sendNickName)
        {
            [_attrStr setTextColor:[SSIMSpUtil getColor:@"f99813"] range:[self.source rangeOfString:self.sendNickName]];
            [_attrStr setFont:[UIFont boldSystemFontOfSize:15] range:[self.source rangeOfString:self.sendNickName]];
        }
        if(self.toTragetName)
        {
            [_attrStr setTextColor:__COLOR_262626__ range:[self.source rangeOfString:@"回复"]];
            [_attrStr setTextColor:[SSIMSpUtil getColor:@"f99813"] range:[self.source rangeOfString:self.toTragetName]];
            
            [_attrStr setFont:[UIFont boldSystemFontOfSize:15] range:[self.source rangeOfString:@"回复"]];
            [_attrStr setFont:[UIFont boldSystemFontOfSize:15] range:[self.source rangeOfString:self.toTragetName]];
        }
        
        self.attrString = _attrStr;
    }
    else
    {
...
-(void)match:(NSString *)source   atCallBack:(BOOL (^)(NSString *))atString   title:(NSAttributedString *)title1   link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    for(int i = 0 ; i < count ; i++)
    {
        ...
        {
            NSMutableString * iconStr=[NSMutableString stringWithFormat:@"%@.png",icon];
            NSMutableDictionary * dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:iconStr,NIMMatchParserImage,[NSNumber numberWithInteger:[text length]+offset],NIMMatchParserLocation,[NSNull null],NIMMatchParserRects,[NSNull null],NIMMatchParserLine, nil];
            [imageArr addObject:dic];
            [text appendString:@" "];
        }
        else
...
#pragma -mark  解析表情图片
    ...
    if([self.images count]>0){
        ...
        NSRange imgLocation =[[nextImage objectForKey:NIMMatchParserLocation] rangeValue];
        ...
        for (id lineObj in lines) { //5
            ...
            for (id runObj in (__bridge NSArray *)CTLineGetGlyphRuns(line1)) { //6
                ...
                if ( runRange.location==imgLocation.location) { //7
                    ...
                    [dic setObject:[NSValue valueWithCGRect:runBounds] forKey:NIMMatchParserRects];
                    [dic setObject:[NSNumber numberWithInt:lineIndex] forKey:NIMMatchParserLine];
                    //load the next image //12
                    
                    imgIndex++;
                    if (imgIndex < [self.images count]) {
                        nextImage = [self.images objectAtIndex: imgIndex];
                        imgLocation = [[nextImage objectForKey: NIMMatchParserLocation] rangeValue];
                    }else{
...
#pragma -mark  解析网址链接
    ...
    if([self.links count]>0){
        ...
        NSRange linkRange =[[nextLink objectForKey:NIMMatchParserRange] rangeValue];
        int lineIndex = 0;
        for (id lineObj in lines) { //5
            CTLineRef line1 = (__bridge CTLineRef)lineObj;
            for (id runObj in (__bridge NSArray *)CTLineGetGlyphRuns(line1)) { //6
                ...
                if ( runRange.location>=linkRange.location&&runRange.location<(linkRange.location+linkRange.length)) { //7
                    ...
                    NSMutableArray * rects=[dic objectForKey:NIMMatchParserRects];
                    ...
                    if((runRange.location+runRange.length)>=(linkRange.location+linkRange.length)){
                        ...
                        if (linkIndex < [self.links count]) {
                            ...
                            linkRange = [[nextLink objectForKey: NIMMatchParserRange] rangeValue];
                        }else{
...
-(void)matchLink:(NSString*)text attrString:(NSMutableAttributedString*)attStr offset:(int)offset link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    if(self.nickNameLink){
        [self matchNickNameLink:text attrString:attStr offset:offset link:link];
    }
    
    [_links sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
        NSDictionary * dic1=obj1;
        NSDictionary * dic2=obj2;
        NSRange range1=((NSValue *)[dic1 objectForKey:NIMMatchParserRange]).rangeValue;
        NSRange range2=((NSValue *)[dic2 objectForKey:NIMMatchParserRange]).rangeValue;
...
//匹配昵称
-(void)matchNickNameLink:(NSString*)source attrString:(NSMutableAttributedString*)attrString1 offset:(int)offset link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    if(self.sendNickName)
    {
        NSRange range = [source rangeOfString:self.sendNickName];
        if (range.location != NSNotFound) {
            NSMutableDictionary * dic= [NSMutableDictionary dictionaryWithObjectsAndKeys:self.sendNickName,NIMMatchParserString,[NSValue valueWithRange:NSMakeRange(range.location+offset,range.length)],NIMMatchParserRange,[[NSMutableArray alloc]init],NIMMatchParserRects,NIMMatchParserLinkTypeNickName,NIMMatchParserLinkType,nil];
    ...
    if(self.toTragetName)
    {
        NSRange range = [source rangeOfString:self.toTragetName];
        if (range.location != NSNotFound)
        {
            NSMutableDictionary * dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:self.toTragetName,NIMMatchParserString,[NSValue valueWithRange:NSMakeRange(range.location+offset,range.length)],NIMMatchParserRange,[[NSMutableArray alloc]init],NIMMatchParserRects,NIMMatchParserLinkTypeNickName,NIMMatchParserLinkType,nil];
            [_links addObject:dic];
        }
    }
...
-(void)matchUrlLink:(NSString*)source attrString:(NSMutableAttributedString*)attrString1 offset:(int)offset link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    for( NSTextCheckingResult * result in array)
    {
        if(link==nil)
        {
            ...
            CTFontRef fontRef=CTFontCreateWithName((__bridge CFStringRef)(self.font.fontName),self.font.pointSize,NULL);
            NSDictionary *attribute=[NSDictionary dictionaryWithObjectsAndKeys:(__bridge id)fontRef,kCTFontAttributeName,(id)self.keyWorkColor.CGColor,kCTForegroundColorAttributeName,[NSNumber numberWithInt:kCTUnderlineStyleSingle],kCTUnderlineStyleAttributeName,nil];
        ...
        NSString * string=[source substringWithRange:result.range];
        NSMutableDictionary * dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:string,NIMMatchParserString,[NSValue valueWithRange:NSMakeRange(result.range.location+offset,result.range.length)],NIMMatchParserRange,[[NSMutableArray alloc]init],NIMMatchParserRects,NIMMatchParserLinkTypeUrl,NIMMatchParserLinkType,nil];
        [_links addObject:dic];
    }
}
...
-(void)matchMobieLink:(NSString*)source attrString:(NSMutableAttributedString*)attrString1 offset:(int)offset link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    for( NSTextCheckingResult * result in array)
    {
        ...
        NSString * string=[source substringWithRange:result.range];
        NSMutableDictionary * dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:string,NIMMatchParserString,[NSValue valueWithRange:NSMakeRange(result.range.location+offset,result.range.length)],NIMMatchParserRange,[[NSMutableArray alloc]init],NIMMatchParserRects,NIMMatchParserLinkTypeMobie,NIMMatchParserLinkType,nil];
        [_links addObject:dic];
    }
}
//匹配座机号
-(void)matchPhoneLink:(NSString*)source attrString:(NSMutableAttributedString*)attrString1 offset:(int)offset link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    for( NSTextCheckingResult * result in array){
        ...
        NSMutableDictionary * dic=[NSMutableDictionary dictionaryWithObjectsAndKeys:string,NIMMatchParserString,[NSValue valueWithRange:NSMakeRange(result.range.location+offset,result.range.length)],NIMMatchParserRange,[[NSMutableArray alloc]init],NIMMatchParserRects,NIMMatchParserLinkTypePhone,NIMMatchParserLinkType,nil];
        [_links addObject:dic];
    }
}
