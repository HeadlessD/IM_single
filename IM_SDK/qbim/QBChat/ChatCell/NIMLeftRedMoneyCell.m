...
#import "NIMLeftRedMoneyCell.h"

@implementation NIMLeftRedMoneyCell
...
- (void)updateWithRecordEntity:(ChatEntity *)recordEntity
{
    ...
    if (dic) {
        ...
        NSString * redType =[NSString stringWithFormat:@"%@",dic[@"type"]];
...
- (void)makeConstraints{
    ...
    [self.iconView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.leading.equalTo(self.avatarBtn.mas_trailing).with.offset(20);
    ...
    if (self.showName) {
        ...
        [self.bubble mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.iconView.mas_leading).with.offset(3);
            make.size.mas_equalTo(CGSizeMake(16, 16));
            make.centerY.equalTo(self.avatarBtn.mas_bottom);
        }];
        
        [self.paoButton mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.vNameLabel.mas_bottom).with.offset(0);
            make.leading.equalTo(self.iconView.mas_leading).with.offset(0);
...
- (UIImageView *)bubble{
    if (!_bubble) {
        ...
        _bubble.image = IMGGET(@"nim_bubble_left");
