...
@interface NIMCameraManger ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureMetadataOutputObjectsDelegate>
...
@implementation NIMCameraManger
        ...
        if([self.cmmotionManager isAccelerometerAvailable]) {
            [self.cmmotionManager startAccelerometerUpdatesToQueue:[NSOperationQueue currentQueue] withHandler:^(CMAccelerometerData * _Nullable accelerometerData, NSError * _Nullable error) {
                if (accelerometerData.acceleration.x >= 0.75) {//home button left
                    ...
                    NSLog(@"角度：%f，%f,%f",accelerometerData.acceleration.x,accelerometerData.acceleration.y,accelerometerData.acceleration.z);
                    return;
                }
            }];
        }
...
{
    __block UIDeviceOrientation  takeOrientation = _currentOrientation;
...
//        UIDeviceOrientation orientation = [UIDevice currentDevice].orientation;
        if (takeOrientation != UIDeviceOrientationPortrait) {
            CGFloat degree = 0;
            if (takeOrientation == UIDeviceOrientationPortraitUpsideDown) {
                degree = 180;// M_PI;
            } else if (takeOrientation == UIDeviceOrientationLandscapeLeft) {
                degree = -90;// -M_PI_2;
            } else if (takeOrientation == UIDeviceOrientationLandscapeRight) {
                degree = 90;// M_PI_2;
            }
//            croppedImage = [croppedImage rotatedByDegrees:degree];
//            scaledImage = [scaledImage rotatedByDegrees:degree];
//            originImage = [self fixOrientation:takeOrientation image:originImage];
        }
        if (block) {
            block(originImage,nil,nil);
        }
    }]; 
}

- (UIImage *)fixOrientation:(UIDeviceOrientation)imageOrientation image:(UIImage *)image {
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    if (self.isStartGetImage) {
        ...
        CGSize size = CGSizeMake(squareLength*2, previewLayerH*2);
        UIImage *scaledImage = [originImage nim_resizedImageWithContentMode:UIViewContentModeScaleAspectFill bounds:size interpolationQuality:kCGInterpolationHigh];
        CGRect cropFrame = CGRectMake((scaledImage.size.width - size.width) / 2, (scaledImage.size.height - size.height) / 2, size.width, size.height);
        UIImage *croppedImage = [scaledImage nim_croppedImage:cropFrame];
        UIDeviceOrientation orientation = [UIDevice currentDevice].orientation;
        if (orientation != UIDeviceOrientationPortrait) {
            ...
            croppedImage = [croppedImage nim_rotatedByDegrees:degree];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
