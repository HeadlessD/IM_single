...
- (void)zh_videoCompressionWithVideoUrl:(NSURL *)videoUrl {
    ...
    // 创建导出视频的URL
    NSString *fileName = @"tempLow.mov";
    int64_t msgid = [NIMStringComponents createMessageid:_chatType];
    if (_chatType != 0) {
        fileName = [NSString stringWithFormat:@"%lld.mov",msgid];
    }
    NSString *filePath = [[[NIMCoreDataManager currentCoreDataManager]recordPathMov] stringByAppendingPathComponent:fileName];
    ...
    [session exportAsynchronouslyWithCompletionHandler:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [ZHPlayVideoView zh_playVideoWithUrl:url isShow:YES completion:^(ZHActionButtonType btnType) {
                if (btnType == ZHActionButtonCancel) { // 取消了，重新录制
                    ...
                    [weakSelf dismissViewControllerAnimated:YES completion:^{
                        ...
                        if (weakSelf.finishBlock) {
                            weakSelf.finishBlock(url, msgid);
                        }
                    }];
                }
            }];
