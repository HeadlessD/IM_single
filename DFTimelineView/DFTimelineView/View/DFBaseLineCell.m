...
//    self.backgroundColor = [UIColor colorWithWhite:0.8 alpha:1];
    ...
    if (_deleteBtn == nil) {
        ...
        _deleteBtn.titleLabel.font = LocationLabelFont;
        [_deleteBtn setTitle:@"删除" forState:UIControlStateNormal];
        [_deleteBtn setTitleColor:[UIColor colorWithRed:35/255.0 green:83/255.0 blue:120/255.0 alpha:1.0] forState:UIControlStateNormal];
...
-(void)updateWithItem:(DFBaseLineItem *)item forRow:(NSInteger)row
{
    self.item = item;
    
    if (self.item.userId == OWNERID) {
        _deleteBtn.hidden = NO;
    }else{
        _deleteBtn.hidden = YES;
    }
    
    [_userAvatarView sd_setImageWithURL:[NSURL URLWithString:USER_ICON_URL(item.userId)] placeholderImage:__IMAGE_PLACEHODER_USERICON__];
    FDListEntity *fd = [FDListEntity instancetypeFindFriendId:item.userId];
    NSString *name = @"";
    if (!IsStrEmpty(fd.fdRemarkName)) {
        name = fd.fdRemarkName;
    } else {
        VcardEntity *vcard = [VcardEntity instancetypeFindUserid:item.userId];
...
-(void) updateLocationLikeComment:(CGFloat)height
{
    ...
    if (self.item.location != nil && ![self.item.location isEqualToString:@""]) {
        ...
        CGSize textSize = [self.item.location boundingRectWithSize:CGSizeMake(100, height) options:NSStringDrawingTruncatesLastVisibleLine attributes:attributes context:nil].size;
        _locationLabel.hidden = NO;
        _locationLabel.frame = CGRectMake(x, y, textSize.width, height);
...
{
    if (item.cellHeight != 0 && !item.isRefresh) {
        NSLog(@"重用高度 %f", item.cellHeight);
        return item.cellHeight;
    }
    NSLog(@"高度前：%f",[[NSDate date] timeIntervalSince1970]);
    CGFloat height = [self getCellHeight:item];
    item.cellHeight = height;
    NSLog(@"计算高度 %f 是否：%d", item.cellHeight,item.isRefresh);
    NSLog(@"高度后：%f",[[NSDate date] timeIntervalSince1970]);
    return height;
}
...
-(UINavigationController *)getController
{
    ...
    UIViewController *controller = [SSIMSpUtil topViewController];;
...
{
    if ([_delegate respondsToSelector:@selector(onClickDelete:)]) {
        [_delegate onClickDelete:self.item.itemId];
    }
}
...
{
    if (_delegate != nil && [_delegate respondsToSelector:@selector(onDeleteComment:)]) {
        [_delegate onDeleteComment:commentId];
    }
}
@end
