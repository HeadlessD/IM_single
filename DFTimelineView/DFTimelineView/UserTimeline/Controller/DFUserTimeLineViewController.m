...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [NIMMomentsManager sharedInstance].user_arr.count;
}


-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    DFBaseUserLineItem *item = [[NIMMomentsManager sharedInstance].user_arr objectAtIndex:indexPath.row];
...
-(void)addItem:(DFBaseUserLineItem *)item
{
    ...
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:(item.ts/1000000)];
